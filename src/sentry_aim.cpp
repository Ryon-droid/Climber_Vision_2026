...
int main(int argc, char *argv[])
{
    ...
    Eigen::Matrix3d R_gimbal2imubody_ = Eigen::Matrix<double, 3, 3, Eigen::RowMajor>(R_gimbal2imubody_data.data());
...
// cv::putText(draw_img, fmt::format("{}", use_tradition ? "CV" : "YOLO"), {10, 30}, cv::FONT_HERSHEY_SIMPLEX, 1, {0, 255, 0}, 2);
        ...
        if (!targets.empty()) {
            ...
            // use command values returned by aimer.aim() call above
            data["target_yaw"] = command.yaw * 57.3;
            data["target_pitch"] = command.pitch * 57.3 ;
            // compute yaw/pitch from the latest IMU quaternion instead of accessing private members
            
            // 计算云台的欧拉角
            // Eigen::Vector3d ypr = tools::eulers(solver.R_gimbal2world(), 2, 1, 0) * 57.3;  // degree

            data["yaw"] = gimbal_pos[0] * 57.3;
            data["pitch"] = gimbal_pos[1] * 57.3;
            
            data["fire"] = command.shoot ? 1 : 0;
            data["err_yaw"] = (gimbal_pos[0]-command.yaw) * 57.3;
            data["err_pitch"] = (gimbal_pos[1]-command.pitch) * 57.3;
            plotter.plot(data);
            // tools::logger()->info("yaw_err: {}, pitch_err:{}",(gimbal_pos[0]-command.yaw) * 57.3,(gimbal_pos[1]-command.pitch) * 57.3);
